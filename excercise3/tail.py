...
def add_shared_keys_tr(shared, dicts, result):
    if not shared:
        return result
    ...
    return add_shared_keys_tr(shared, dicts, result)

def add_non_shared_keys_tr(non_shared, dicts, result):
    if not non_shared:
        return result
    
    key = non_shared.pop(0)
    for d in dicts:
        if key in d:
            if key in result:
    ...
    return add_non_shared_keys_tr(non_shared, dicts, result)
...
def add2dicts(d1, d2, d3):
    ...
    result = add_shared_keys_tr(shared, dicts, {})
    result = add_non_shared_keys_tr(non_shared, dicts, result)
